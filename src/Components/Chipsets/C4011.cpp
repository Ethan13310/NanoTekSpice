...
nts::C4011::C4011(std::string const &)
{
...
nts::Tristate nts::C4011::compute(std::size_t pin)
{
	...
	switch (pin) {
	case 3:
		return m_pins.compute(gate::Nand, 1, 2);
	case 4:
		return m_pins.compute(gate::Nand, 5, 6);
	case 10:
		return m_pins.compute(gate::Nand, 8, 9);
	case 11:
		return m_pins.compute(gate::Nand, 12, 13);
	default:
...
{
	std::cout << "[4011] " << utils::addr(this) << std::endl;
	m_pins.dump();
}
